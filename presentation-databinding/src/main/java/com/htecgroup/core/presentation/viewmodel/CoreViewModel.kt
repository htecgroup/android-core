...
/**
 * Base [ViewModel] class, where [ActionCodeT] is enum representing actions.
 */
@Suppress("unused")
abstract class CoreViewModel<ActionCodeT> : ICoreViewModel<ActionCodeT>, ViewModel() {

    private val action by lazy { SingleLiveEvent<Action<ActionCodeT>>() }
    private val status by lazy { MutableLiveData(Status()) }
