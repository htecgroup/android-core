...
data class PostDetailsDestination(val id: Int) : PersistableNavDestination {
    override val navKeyId: String = NAV_KEY_ID
    override fun encodeParams(): Map<String, String> = mapOf(ID_PARAM to id.toString())
    override fun createEntry(): CoreNavEntry = postDetailsEntry(this)

    companion object : NavKeyFactory<PostDetailsDestination> {
        const val NAV_KEY_ID = "post_detail"
        private const val ID_PARAM = "id"

        override fun fromParams(params: Map<String, String>): PostDetailsDestination {
            val id = params[ID_PARAM]?.toIntOrNull() ?: error("Missing id")
            return PostDetailsDestination(id)
        }
    }
}
